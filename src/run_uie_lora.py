...
class UIETrainingArguments(Seq2SeqTrainingArguments):
    ...
    galore_scale: float = field(default=1.0)
    galore_proj_type: str = field(default="std")
    optim_target_modules: Optional[Union[List[str], str]] = field(default = ".*EncDecAttention.(q|v).*")
    # optim_target_modules: Optional[Union[List[str], str]] = field(default = ".*SelfAttention.(q|v).*, .*EncDecAttention.(q|v).*")
    proj_lora_modules: str = field(default = ".*self_attn.(q_proj|v_proj).loranew_A.*")
...
def main():
    ...
    training_args.deepspeed_config = training_args.deepspeed
    local_rank = int(os.getenv('LOCAL_RANK', 0))
    if local_rank == 0:
        print(f"\n######################\nlora=gpfirst,\ngalore=gpfirst,\nlorarank={model_args.lora_dim},\nlr={training_args.learning_rate},\ngalorerank={training_args.galore_rank}\ngalore lr={training_args.galore_lr}\n######################\n")
    # Setup loggingWS
    logging.basicConfig(
    ...
    for name, param in model.named_parameters():
        ...
        # this module should always be frozen because we change the vocabulary
            ...
            param.requires_grad = False
        elif training_args.optim_target_modules:
            if isinstance(training_args.optim_target_modules, str):
                target_modules = training_args.optim_target_modules.replace(" ", "").split(",")
                for target_module in target_modules:
                    if re.fullmatch(target_module, name):
                        param.requires_grad = True
            else:
                for target_module in training_args.optim_target_modules:
    ...
    if local_rank == 0:
        grad_mat = trainer.get_grad_matrix_from_grad()
        rep_path = os.path.join(peft_model_id, "rep_mats.pkl")
        if os.path.exists(rep_path):
            os.remove(rep_path)
        with open(rep_path, "wb") as f:
            pickle.dump(grad_mat, f)
